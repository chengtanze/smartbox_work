...
-(void)setGrade:(float)grade
{
    ...
    if(_type == DWONTYPE)
    {
        ...
        [progressline addLineToPoint:CGPointMake(pointx,  grade * pointy)];
    }
    else
    {
            [progressline moveToPoint:CGPointMake(pointx, pointy)];
            [progressline addLineToPoint:CGPointMake(pointx, (1 - grade) * pointy)];
    }
...
-(void) animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    switch (_type) {
        ...
        case DWONTYPE:
        {
            UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(0, _grade * pointy-30, self.frame.size.width, 30)];
            [label setTextAlignment:NSTextAlignmentCenter];
            [label setFont:[UIFont systemFontOfSize:10.0f]];
            label.textColor = [UIColor darkGrayColor];
            label.numberOfLines = 0;
